...
export default function PortfolioDialog({ onClose }) {
  ...
  const documentHtml = useMemo(() => portfolioDocument(window.location.origin), [])
  ...
  useEffect(() => {
    ...
    const receiveBookMessage = (event) => {
      ...
      } else if (event.data.type === 'close') closeRef.current()
    }
  ...
  return (
    <dialog ref={dialogRef} className="portfolio-dialog" id="portfolio-dialog" aria-labelledby="portfolio-title" onCancel={(event) => { event.preventDefault(); onClose() }} onClick={closeFromBackdrop}>
      ...
      <div className="portfolio-dialog-body">
        <iframe ref={frameRef} key={attempt} className="portfolio-book-frame" data-ready={frameStatus === 'ready'} srcDoc={documentHtml} title="范钦威建筑作品集翻页画册" onLoad={connectFrame} onError={() => setFrameStatus('error')} />
